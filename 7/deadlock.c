...
int main(int argc, char const *argv[])
{
	...
	maximum = malloc(NUMBER_OF_CUSTOMERS*sizeof(*maximum));
	need = malloc(NUMBER_OF_CUSTOMERS*sizeof(*need));
	allocation = malloc(NUMBER_OF_CUSTOMERS*sizeof(*allocation));
...
void *customer_thread(int n) {
    ...
    while(true){
        ...
        for(int i = 0; i < NUMBER_OF_RESOURCES; i++){
            ...
            check[i] = allocation[n][i];
        }
        if (flag){
        done = release_resources(n, check);
        printf("Customer %d Releases [%s\b]\n", n + 1, req_to_str(check));
        break;
        }
    }
